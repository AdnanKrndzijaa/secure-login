...
require_once dirname(__FILE__) . "/OTPAuth.class.php";
 ...
 */
 ...
 Flight::route('POST /generateQRCode', function () {
    $dao = new UserDao();
    $otp = new OTPAuth($dao);
    $secret = $otp->generateQRCode();
    $qrCodeUri = $otp->createQR($secret);
    Flight::json([
        'secret' => $secret,
        'qrCodeUri' => $qrCodeUri
    ]);
...
Flight::route('POST /validateQRCode', function () {
    $data = Flight::request()->data->getData();
    $dao = new UserDao();
    $otp = new OTPAuth($dao);

    $secret = $dao->getSecret(); // Retrieve the secret from the database or any other source

    $isValid = $otp->validateOTP($data['userInput'], $secret);

    if ($isValid) {
        // Redirect to the login page
        Flight::redirect('../front/login.html');
    } else {
        // Set an error message and redirect back to the QR code page
        Flight::flash('errorMessage', 'Invalid code. Please try again.');
        Flight::redirect('../front/2fa.html');
    }
});
